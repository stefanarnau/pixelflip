...
for s = 1 : length(subject_list)
    ...
    for e = 1 : size(EEG.trialinfo, 1)
        ...
        if EEG.trialinfo(e, 3) == 0
            ...
            % If previous trial is not available...
            else

                % ...it is not a good trial.
                EEG.trialinfo(e, 12) = -1;
                EEG.trialinfo(e, 13) = -1;
...
GA = {};
for s = 1 : length(subject_list)
    chan_time_data = (squeeze(erp_flip0_easy_post0(s, :, :)) + squeeze(erp_flip1_easy_post0(s, :, :))) ./ 2;
...
GA = {};
for s = 1 : length(subject_list)
    chan_time_data = (squeeze(erp_flip0_hard_post0(s, :, :)) + squeeze(erp_flip1_hard_post0(s, :, :))) ./ 2;
    ga_template.avg = chan_time_data;
    GA{s} = ga_template;
end 
