...
dlmwrite([PATH_VEUSZ, 'lineplots_fz.csv'],  [mean(squeeze(erp_easy_asis(:, 11, :)), 1);...
                                             mean(squeeze(erp_easy_flip(:, 11, :)), 1);...
                                             mean(squeeze(erp_hard_asis(:, 11, :)), 1);...
                                             mean(squeeze(erp_hard_flip(:, 11, :)), 1)]);

% Save lineplots at POz
dlmwrite([PATH_VEUSZ, 'lineplots_fcz.csv'], [mean(squeeze(erp_easy_asis(:, 20, :)), 1);...
                                             mean(squeeze(erp_easy_flip(:, 20, :)), 1);...
                                             mean(squeeze(erp_hard_asis(:, 20, :)), 1);...
                                             mean(squeeze(erp_hard_flip(:, 20, :)), 1)]);

dlmwrite([PATH_VEUSZ, 'lineplots_pz.csv'],  [mean(squeeze(erp_easy_asis(:, 48, :)), 1);...
                                             mean(squeeze(erp_easy_flip(:, 48, :)), 1);...
                                             mean(squeeze(erp_hard_asis(:, 48, :)), 1);...
                                             mean(squeeze(erp_hard_flip(:, 48, :)), 1)]);

% Save lineplots at POz
dlmwrite([PATH_VEUSZ, 'lineplots_poz.csv'], [mean(squeeze(erp_easy_asis(:, 57, :)), 1);...
                                             mean(squeeze(erp_easy_flip(:, 57, :)), 1);...
                                             mean(squeeze(erp_hard_asis(:, 57, :)), 1);...
